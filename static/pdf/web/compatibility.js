...
(function checkWindowBtoaCompatibility () {
  ...
  window.btoa = function windowBtoa (chars) {
    ...
    for (i = 0, n = chars.length; i < n; i += 3) {
      ...
      const d1 = b1 >> 2; const d2 = ((b1 & 3) << 4) | (b2 >> 4)
...
(function checkFunctionPrototypeBindCompatibility () {
  ...
  Function.prototype.bind = function functionPrototypeBind (obj) {
    const fn = this; const headArgs = Array.prototype.slice.call(arguments, 1)
  ...
  Object.defineProperty(HTMLElement.prototype, 'dataset', {
    get () {
      ...
      for (let j = 0, jj = this.attributes.length; j < jj; j++) {
        ...
        const key = attribute.name.substring(5).replace(/\-([a-z])/g,
          function (all, ch) {
...
(function checkRangeRequests () {
  ...
  // Support: Android<3.0
  const regex = /Android\s[0-2][^\d]/
  const isOldAndroid = regex.test(navigator.userAgent)
...
(function checkHistoryManipulation () {
  ...
  // Support: Android 2.x
  if (!history.pushState || navigator.userAgent.includes('Android 2.')) {
...
(function checkSetPresenceInImageData () {
  ...
  if (window.CanvasPixelArray) {
    ...
    let polyfill = false; let versionMatch
    ...
    if (polyfill) {
      ...
      contextPrototype.createImageData = function (w, h) {
        const imageData = this._createImageData(w, h)
